...
export const initState = () => {
    ...
    $('#subway-line-for-section').insertAdjacentHTML('beforeend', createStationSelectTemplate(lineNameList));
    $('#subway-line').insertAdjacentHTML('beforeend', createStationSelectTemplate(lineNameList));
...
const handleLineRegister = (e) => {
    ...
    lineList.map((lineInfo) => {
        if (lineInfo.name === lineName) return lineInfo.section.push([upStation, downStation]);
    });
...
const getSelectedLineInfo = () => {
    ...
    const selectedLineInfo = lineList.filter((lineInfo) => lineInfo.name === selectedLine)[0];
